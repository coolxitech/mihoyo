...
class Api extends BaseController
{
    ...
    public function app_login(): Json
    {
        ...
        try {
            ...
            if ($login_data['retcode'] == -3101) {
                ...
                $request = $this->client->post('https://passport-api.mihoyo.com/account/ma-cn-passport/app/loginByPassword', [
                    ...
                    'headers' => [
                        'DS' => Encrypt::newDS(Config('key.cn_app_salt'), $data),
                        'x-rpc-account_version' => Config('key.app_version'),
                        'x-rpc-client_type' => 2,
                        'x-rpc-app_id' => Config('key.app_id'),
        ...
        if ($login_data['retcode'] != 0) {
            return Response::error(-3, '登录错误', [$login_data['message']]);
        }
        ...
        if ($cookies['retcode'] != 0) {
            return Response::error(-5, '获取cookie失败');
        }
    ...
    private function identification_codes(string $gt, string $challenge, string $referer): array
    {
        ...
        //公益打码代码开始
        //        $token = 'xSQyehXmskzSZeR8FIF6Tix3wtzUQYCB';
        ...
        $appkey = 'f91e9485e6de4fe0b6e943244ae41dc7';
        $request = $this->client->get('http://api.rrocr.com/api/integral.html?appkey=' . $appkey);
        $result = $request->getBody()->getContents();
        $integral_data = json_decode($result, true);
        if ($integral_data['status'] == -1) {
            return Response::error(-2, '打码积分查询失败');
        }
        if ($integral_data['integral'] <= 10) {
            Response::error(-2, '积分不足,请联系管理员');
        }
    ...
    private function mihoyo_login(string $username, string $password): array
    {
        ...
        try {
            ...
            if ($try < 5) {
                ...
                throw new Exception('第一次登录,验证码识别失败,' . $e->getMessage());
            }
        ...
        try {
            ...
            foreach ($source_cookies as $cookie) {
                ...
                if ($matches[1] == 'aliyungf_tc' or $matches[1] == 'acw_tc') {
                    continue;
                }//剔除阿里云的Cookie
                $cookies[$matches[1]] = $matches[2];
            }
        ...
        if ($miyoushe_valid_data['retcode'] != 0) {
            return [];
        }
        ...
        if ($mohoyo_user_login_data['code'] != 200) {
            return $mohoyo_user_login_data;
        }
    ...
    public function genshin_signIn(): Json
    {
        ...
        if (empty($cookie)) {
            return Response::error(-1, 'cookie不能为空');
        }
        ...
        if ($game_info['retcode'] != 0) {
            return Response::error(-2, '获取游戏信息失败', $game_info);
        }
        //防止账号下多个游戏角色
        if (count($game_info['data']['list']) == 1) {
            ...
            if ($region == '' or $uid == '') {
                return Response::error(-3, '该账号下拥有多个游戏角色，请输入指定的区域和游戏ID进行匹配.', $game_info['data']['list'], 401);
            }
        ...
        if ($region == '' or $uid == '') {
            return Response::error(-4, '未能匹配到指定游戏账号');
        }
        //签到请求提交
